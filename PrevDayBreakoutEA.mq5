...
input double              InpRiskPercent    = 1.0;             // Risk Alert Threshold (%)
...
//+------------------------------------------------------------------+
//| Get palette colors into output variables                         |
//+------------------------------------------------------------------+
void GetPaletteColors(ENUM_COLOR_PALETTE palette,
                      color &bgColor, color &bullColor, color &bearColor,
                      color &fgColor, color &gridColor, color &volColor, color &lineColor)
{
   switch(palette)
   {
      case PALETTE_ONYX:
         bgColor   = C'18,18,18';
         bullColor = C'200,184,154';
         bearColor = C'122,115,104';
      ...
      case PALETTE_NIGHT:
         bgColor   = C'11,18,32';
         bullColor = C'199,226,247';
         bearColor = C'90,100,114';
      ...
      case PALETTE_ARCTIC:
         bgColor   = C'241,246,251';
         bullColor = C'255,255,255';
         bearColor = C'199,216,230';
      ...
      case PALETTE_SMC:
         bgColor   = C'243,241,236';
         bullColor = C'107,143,122';
         bearColor = C'47,42,37';
         fgColor   = C'80,75,65';
      ...
      case PALETTE_LAVENDER:
         bgColor   = C'244,239,251';
         bullColor = C'255,255,255';
         bearColor = C'122,98,133';
      ...
      case PALETTE_CLAUDE:
         bgColor   = C'31,28,25';
         bullColor = C'211,138,93';
         bearColor = C'126,122,117';
         fgColor   = C'160,155,148';
...
void ConfigureChartColors()
{
   ...
   GetPaletteColors(InpColorPalette, bgColor, bullColor, bearColor, fgColor, gridColor, volColor, lineColor);
...
void CreatePanel()
{
   // Derive accent color from palette for title
   color bgColor, bullColor, bearColor, fgColor, gridColor, volColor, accentColor;
   GetPaletteColors(InpColorPalette, bgColor, bullColor, bearColor, fgColor, gridColor, volColor, accentColor);

   // Panel background — slightly offset from chart BG
   ...
   ObjectSetInteger(0, panelBG, OBJPROP_XSIZE,        220);
   ObjectSetInteger(0, panelBG, OBJPROP_YSIZE,        95);
   ObjectSetInteger(0, panelBG, OBJPROP_BGCOLOR,      bgColor);
   ObjectSetInteger(0, panelBG, OBJPROP_BORDER_COLOR, accentColor);
   ...
   ObjectSetInteger(0, panelTitle, OBJPROP_COLOR,     accentColor);
...
void UpdatePanel()
{
   ...
   // Determine light vs dark theme for profit/loss coloring
   bool isLight = (InpColorPalette == PALETTE_ARCTIC   ||
                   InpColorPalette == PALETTE_SMC       ||
                   InpColorPalette == PALETTE_LAVENDER);

   color colorGreen  = isLight ? C'34,139,70'  : C'72,199,142';
   color colorRed    = isLight ? C'180,40,40'  : C'220,95,95';
   color colorNeutral = fgColor;
   ...
   ObjectSetInteger(0, panelRisk, OBJPROP_COLOR, (riskPct <= InpRiskPercent) ? colorGreen : colorRed);
   ...
   if(todayProfit > 0)
      ObjectSetInteger(0, panelProfit, OBJPROP_COLOR, colorGreen);
   else if(todayProfit < 0)
      ObjectSetInteger(0, panelProfit, OBJPROP_COLOR, colorRed);
   else
      ObjectSetInteger(0, panelProfit, OBJPROP_COLOR, colorNeutral);

   // --- Status row ---
   string statusText;
   if(!IsWeekday())         statusText = "Weekend";
   else if(IsCloseTime())   statusText = "Closed";
   else if(!IsTradingStartTime()) statusText = "Waiting...";
   else                     statusText = "Active";

   ObjectSetString(0,  panelStatus, OBJPROP_TEXT,  statusText);
   ObjectSetInteger(0, panelStatus, OBJPROP_COLOR, fgColor);
...
bool IsLondonSessionTime()
{
   ...
   if(londonEnd < londonStart) londonEnd += 86400;
...
void CalculateLondonSessionHighLow()
{
   ...
   if(startBar < endBar) { int temp = startBar; startBar = endBar; endBar = temp; }
...
void GetTodayOpenPrice()
{
   ...
   todayOpenPrice = (copied > 0) ? open[0] : SymbolInfoDouble(_Symbol, SYMBOL_BID);
}
...
double GetPipValue()
{
   return GetPipInPoints() * SymbolInfoDouble(_Symbol, SYMBOL_POINT);
}
...
double GetPipInPoints()
{
   ...
   if(isXAU)
   {
      if(point > 0) return 0.10 / point;
      return (digits == 3) ? 100.0 : 10.0;
   }
   else if(isIndex)
   {
      if(point > 0) return 1.0 / point;
      return 1.0;
   }
   else if(isJPY)
      ...
      if(position.SelectByIndex(i))
      {
         if(position.Symbol() == _Symbol && position.Magic() == InpMagicNumber)
         {
            ...
            if(profitPips >= InpTrailingStart)
            {
               ...
               if(posType == POSITION_TYPE_BUY  && newSL >= bidPrice - minStop) continue;
               if(posType == POSITION_TYPE_SELL && newSL <= askPrice + minStop) continue;

               bool shouldUpdate = false;
               if(posType == POSITION_TYPE_BUY)
                  shouldUpdate = (currentSL == 0 || newSL > currentSL + trailingStep);
               else
                  shouldUpdate = (currentSL == 0 || (currentSL - newSL) >= trailingStep);
...
//+------------------------------------------------------------------+
//| Cancel conflicting pending orders when a position is open        |
//+------------------------------------------------------------------+
   ...
   for(int posIdx = PositionsTotal() - 1; posIdx >= 0; posIdx--)
   {
      if(position.SelectByIndex(posIdx))
      {
         if(position.Symbol() == _Symbol && position.Magic() == InpMagicNumber)
         {
            ...
            {
               if(posType == POSITION_TYPE_BUY) { minPrice = posSL;    maxPrice = posEntry; }
               else                              { minPrice = posEntry; maxPrice = posSL;    }
            }
            else if(posTP > 0)
            {
               if(posType == POSITION_TYPE_BUY) { minPrice = posEntry; maxPrice = posTP;    }
               else                              { minPrice = posTP;    maxPrice = posEntry; }
            }
            else continue;
...
{
   // Use palette bull color as the line accent
   color bgColor, bullColor, bearColor, fgColor, gridColor, volColor, lineColor;
   GetPaletteColors(InpColorPalette, bgColor, bullColor, bearColor, fgColor, gridColor, volColor, lineColor);

   ObjectDelete(0, lineHigh);
   ObjectCreate(0,  lineHigh, OBJ_HLINE, 0, 0, prevDayHigh);
   ObjectSetInteger(0, lineHigh, OBJPROP_COLOR, lineColor);
   ...
   ObjectSetInteger(0, lineLow, OBJPROP_COLOR, lineColor);
...
void DrawLondonLines()
{
   ...
   ObjectSetInteger(0, lineLondonHigh, OBJPROP_COLOR, bullColor);
   ...
   ObjectSetInteger(0, lineLondonLow, OBJPROP_COLOR, bullColor);
